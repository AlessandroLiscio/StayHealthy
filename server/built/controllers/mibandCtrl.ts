...
export class MibandCtrl extends TableCtrl {
    ...
    public async postData(req: Request): Promise<any> {
        ...
        if (!(this.result instanceof Error || this.result instanceof CustomError)) {
            ...
            this.result = await this.putLastFetchDate(req.query.patient_ssn, this.result);
            console.log("newLastFetchDate: ", newLastFetchDate)
            if (!(this.result instanceof Error || this.result instanceof CustomError)) { this.result = newLastFetchDate }
        // else, remoove the data on the database starting from the current last fetch date, and return the current last fetch date
        } else {
            this.result = await this.deleteFromLastFetchDate(req.query.patient_ssn, currentLastFetchDate)
            if (!(this.result instanceof Error || this.result instanceof CustomError)) { this.result = currentLastFetchDate }
        }
        return this.result;
    }
